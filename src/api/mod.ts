...
export default function () {
  const token = Deno.env.get("NETBSD_UP_API_TOKEN") ||
    crypto.randomUUID();
  const { flags } = parseFlags(Deno.args);

  if (!Deno.env.get("NETBSD_UP_API_TOKEN")) {
    console.log(`Using API token: ${token}`);
  } else {
    console.log(
      `Using provided API token from environment variable NETBSD_UP_API_TOKEN`,
    );
  }
  ...
  const port = Number(
    flags.port || flags.p ||
      (Deno.env.get("NETBSD_UP_PORT")
        ? Number(Deno.env.get("NETBSD_UP_PORT"))
        : 8892),
  );
